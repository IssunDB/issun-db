...
fn evaluate_match(
    ...
    let seed_var = pattern.node.variable.clone().unwrap_or_else(|| "_seed".to_string());
    ...
    for path in &current_paths {
        for &cand in &seed_candidates {
            ...
            if let Some(existing) = new_path.insert(seed_var.clone(), cand) {
                if existing != cand {
                    continue;
                }
            }
            ...
            for (rel_pat, node_pat) in &pattern.rels {
                ...
                let rel_var = rel_pat.variable.clone().unwrap_or_else(|| "_rel".to_string());
                let target_var = node_pat.variable.clone().unwrap_or_else(|| "_target".to_string());

                for b_path in branch_paths {
                    let current_node = b_path[&seed_var];
                    let neighbors = if rel_pat.is_incoming {
                        graph.in_neighbors(current_node).map_err(|e| e.to_string())?
                    } else {
                        graph.out_neighbors(current_node).map_err(|e| e.to_string())?
                    };
                    ...
                    for (neigh_node, _edge_id, type_id) in neighbors {
                        ...
                        if let Some(ref label) = node_pat.label {
                            let label_nodes = graph.nodes_by_label(label).map_err(|e| e.to_string())?;
                        ...
                        step_path.insert(rel_var.clone(), neigh_node); // Keep track of traversal step
                        if let Some(existing) = step_path.insert(target_var.clone(), neigh_node) {
                            if existing != neigh_node {
                                continue;
                            }
                        }
...
fn check_inline_properties(
    ...
    let actual_json: serde_json::Value = rmp_serde::from_slice(&record.props).map_err(|e| e.to_string())?;
...
fn evaluate_expr(
    ...
    match expr {
        ...
        Expr::Param(p) => params.get(p).cloned().ok_or_else(|| format!("missing parameter: {}", p)),
        Expr::Prop(var, prop) => {
            let &node_id = path.get(var).ok_or_else(|| format!("unbound variable: {}", var))?;
            let record = graph.get_node(node_id).map_err(|e| e.to_string())?.ok_or_else(|| format!("node not found: {}", node_id))?;
            let actual_json: serde_json::Value = rmp_serde::from_slice(&record.props).map_err(|e| e.to_string())?;
            Ok(actual_json.get(prop).cloned().unwrap_or(serde_json::Value::Null))
        }
    }
}
...
fn execute_create(
    ...
    let label = create.pattern.node.label.clone().unwrap_or_else(|| "Node".to_string());
    let seed_id = graph.add_node(&label, &props_map).map_err(|e| e.to_string())?;
    ...
    for (rel_pat, node_pat) in &create.pattern.rels {
        ...
        let target_id = graph.add_node(&target_label, &target_props).map_err(|e| e.to_string())?;

        let rel_type = rel_pat.rel_type.clone().unwrap_or_else(|| "EDGE".to_string());
        let empty_props: HashMap<String, serde_json::Value> = HashMap::new();

        if rel_pat.is_incoming {
            graph.add_edge(target_id, created_node_id, &rel_type, &empty_props).map_err(|e| e.to_string())?;
        } else {
            graph.add_edge(created_node_id, target_id, &rel_type, &empty_props).map_err(|e| e.to_string())?;
        }
    ...
    for path in bound_paths {
        ...
        if is_match {
            for set_item in &set.set_items {
                let &node_id = path.get(&set_item.variable).ok_or_else(|| format!("unbound variable: {}", set_item.variable))?;
                let record = graph.get_node(node_id).map_err(|e| e.to_string())?.ok_or_else(|| format!("node not found: {}", node_id))?;
                let mut actual_json: serde_json::Value = rmp_serde::from_slice(&record.props).map_err(|e| e.to_string())?;
                ...
                graph.update_node(node_id, &label_name, &actual_json).map_err(|e| e.to_string())?;
            }
            matched_count += 1;
        }
    }
