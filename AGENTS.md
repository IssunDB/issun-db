...
IssunDB is an embedded graph and vector database for AI/RAG workloads, written in Rust.
Priorities, in order:
...
- Keep the workspace modular: `issundb-core` owns storage, `issundb-cypher` owns the query layer, `issundb-rag` owns GraphRAG primitives, and
  `issundb` is the public facade. Do not import across those boundaries in the wrong direction.
...
The current tree covers phase 1; this layout describes the target structure as the project grows.
...
- `crates/issundb-core/`: storage engine. Public surface is `Graph` and the schema types.
    ...
    - `src/csr.rs` *(phase 3)*: in-memory CSR snapshot, rebuilt in the background and swapped via `arc-swap`.
...
- `crates/issundb/`: public facade. Re-exports the deliberate public surface from `issundb-core`, `issundb-cypher`, and `issundb-rag`. Do not
  re-export internal storage types like `Storage` or `AdjList`.
...
- `crates/issundb-cli/`: interactive REPL binary. Entry point for manual exploration and demos.
- `examples/`: standalone Rust examples (`graphrag_quickstart.rs`, `neo4j_migration.rs`, `load_ldbc.rs`).
- `benches/`: Criterion benchmarks (`ldbc_subset.rs`).
- `tests/conformance/`: openCypher TCK subset integration tests.
...
- Cypher conformance tests belong in `tests/conformance/` and are gated on the `ISSUNDB_CONFORMANCE=1` environment variable so the default
...
- Benchmark targets live in `benches/`; do not add `#[bench]` to source files.

## Architecture Constraints

- Adjacency is stored as LMDB `DUPSORT + DUPFIXED`: each duplicate value under a node key is one raw `AdjEntry` (20 bytes). A single `db.put` appends one entry in O(log n); there is no read-modify-write of a blob.
- Secondary indexes (`label_idx`, `type_idx`) use 12-byte composite keys `(u32 BE, u64 BE)` stored in plain LMDB databases with `Unit` values. Prefix-range scans via `prefix_iter` enumerate all nodes or edges for a given label or type in ascending ID order.
- The CSR snapshot (phase 3) is the hot read path for traversal. GraphBLAS operates on the CSR snapshot for pattern matching and multi-source
  expansion. The two coexist: use CSR for simple BFS and neighbor queries; use GraphBLAS for Cypher plan execution and large-scale expansions.
...
- Heavy dependencies (GraphBLAS, usearch, chumsky) are commented out in `Cargo.toml` and are enabled phase by phase. Do not enable them ahead of their
  phase.
...
GraphRAG primitives. Depends on `issundb-core`; must not be imported by `issundb-core`. All retrieve functions are free functions, not methods on `Graph`, to preserve the crate boundary.
...
Internal to `issundb-core`. Owns the LMDB environment and all eight sub-databases: `nodes`, `edges`, `out_adj`, `in_adj`, `label_idx`, `type_idx`, `vectors`, and `meta`. Do not
...
`Storage`, `AdjList`, and the `storage` module are `pub(crate)` within `issundb-core`. The `issundb` facade re-exports only `Graph`, `Error`, and the
schema ID types. Do not add a "just for now" re-export anywhere else; add a deliberate testing helper in `issundb-core` if a test needs internal
access.
...
1. Identification of whether this is a storage, query, vector, GraphRAG, bindings, or docs change.
...
- Public API docs are generated from `rustdoc` on `crates/issundb/src/lib.rs`. Keep that module focused on the deliberate public surface; do not
  re-export `Storage`, `AdjList`, or other internals.
